    ...
    <div class="bg-[#181824] border border-amber-500/40 rounded-3xl max-w-md w-full p-6 text-white shadow-2xl space-y-4">
      ...
      <div class="text-center space-y-1.5">
        ...
        <p class="text-xs text-zinc-300 leading-relaxed">
          To protect Ghanaian jobseekers and attendees from fraud, SellerFlow requires all users to complete <b>Ghana Card identity verification</b> before ${esc(actionType)}.
        </p>
      </div>
...
async function renderExploreJobs(container) {
    ...
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-12 gap-3">
        <div class="sm:col-span-6 relative">
          <input type="text" id="jobSearchInput" value="${esc(activeJobSearch)}" placeholder="Search job title, skills, company..." class="w-full bg-[#14141e] border border-zinc-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-zinc-500 focus:border-gold/60 focus:outline-none" />
        </div>
  ...
  $('jobRegionSelect')?.addEventListener('change', (e) => {
    ...
    <div class="bg-[#14141e] hover:bg-[#181826] border border-zinc-800/80 hover:border-amber-500/40 rounded-2xl p-4 sm:p-5 transition shadow-sm space-y-3">
      <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div class="space-y-1 min-w-0">
          ...
          <div class="flex items-center gap-3 text-xs text-zinc-400 flex-wrap">
            <span class="font-bold text-zinc-300">🏢 ${esc(job.companyName)}</span>
            <span>📍 ${esc(job.region || 'Ghana')} (${esc(job.locationType || 'On-site')})</span>
            <span class="text-amber-300 font-semibold">💰 ${esc(formattedSalary)}</span>
          </div>
        </div>
      ...
      <div class="flex items-center justify-between text-[11px] text-zinc-500 pt-1 border-t border-zinc-800/50">
        <div class="flex items-center gap-3">
          <span>Category: <b class="text-zinc-400">${esc(job.category || 'General')}</b></span>
          <span>Type: <b class="text-zinc-400">${esc(job.employmentType || 'Full-time')}</b></span>
        </div>
  ...
  modal.querySelector('#postJobForm').onsubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        companyName: $('jobCompany').value.trim(),
        category: $('jobCategory').value,
        region: $('jobRegion').value,
        locationType: $('jobLocationType').value,
        salaryMin: Number($('jobSalaryMin').value) || 0,
        salaryMax: Number($('jobSalaryMax').value) || 0,
        salaryPeriod: $('jobSalaryPeriod').value,
        description: $('jobDesc').value.trim(),
        requirements: $('jobReqs').value.trim(),
        scamWarningAcknowledged: $('postScamAck').checked
      };
...
async function openApplyJobModal(jobId) {
  ...
  modal.innerHTML = `
    <div class="bg-[#181824] border border-gold/40 rounded-3xl max-w-lg w-full p-6 text-white shadow-2xl space-y-4 max-h-[92vh] flex flex-col">
      <div class="flex items-center justify-between pb-3 border-b border-zinc-800">
        <div>
          <h3 class="text-base font-black text-white">Apply for ${esc(jobData?.title || 'Position')}</h3>
          <p class="text-[11px] text-zinc-400">at ${esc(jobData?.companyName || 'Verified Employer')}</p>
        </div>
      ...
      <form id="applyJobForm" class="flex-1 overflow-y-auto pr-2 space-y-3.5 text-xs text-zinc-300">
        <div>
          ...
          <input type="text" id="applicantName" required value="${esc(currentProfile?.name || currentUser?.displayName || '')}" class="w-full bg-[#12121a] border border-zinc-800 rounded-xl px-3 py-2 text-white text-xs focus:border-gold/60 focus:outline-none" />
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label class="block font-bold text-zinc-300 mb-1">Email Address *</label>
            <input type="email" id="applicantEmail" required value="${esc(currentUser?.email || '')}" class="w-full bg-[#12121a] border border-zinc-800 rounded-xl px-3 py-2 text-white text-xs focus:border-gold/60 focus:outline-none" />
          </div>
          <div>
  ...
  modal.querySelector('#applyJobForm').onsubmit = async (e) => {
    ...
    try {
      const fileInput = $('applicantCvFile');
      let cvBase64 = null;
      let cvFileName = '';
      let cvFileType = '';
      ...
      const payload = {
        ...
        coverLetter: $('applicantCover').value.trim(),
        cvBase64,
        cvFileName,
        cvFileType,
        scamWarningAcknowledged: $('applicantScamAck').checked
      };
      ...
      <div class="bg-[#181824] border border-zinc-700 rounded-3xl max-w-2xl w-full p-6 text-white shadow-2xl space-y-4 max-h-[92vh] flex flex-col">
        <div class="flex items-start justify-between pb-3 border-b border-zinc-800">
          <div class="space-y-1">
            ...
            <p class="text-xs font-bold text-zinc-300">🏢 ${esc(job.companyName)} · 📍 ${esc(job.region)} (${esc(job.locationType)})</p>
          </div>
        ...
        <div class="flex-1 overflow-y-auto pr-2 space-y-4 text-xs leading-relaxed text-zinc-300">
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5 p-3.5 rounded-2xl bg-[#12121a] border border-zinc-800">
            <div>
              ...
              <span class="text-amber-300 font-bold">${esc(formattedSalary)}</span>
            </div>
            <div>
              <span class="text-[10px] text-zinc-500 block uppercase font-bold">Job Type</span>
              <span class="text-white font-semibold">${esc(job.employmentType || 'Full-time')}</span>
            </div>
            <div>
              <span class="text-[10px] text-zinc-500 block uppercase font-bold">Category</span>
              <span class="text-white font-semibold">${esc(job.category || 'General')}</span>
            </div>
          </div>

          <div>
            <h4 class="font-bold text-white mb-1.5 text-sm">Job Description</h4>
            <div class="whitespace-pre-line text-zinc-300">${esc(job.description)}</div>
          </div>

          ${job.requirements ? `
            <div>
              <h4 class="font-bold text-white mb-1.5 text-sm">Requirements & Skills</h4>
              <div class="whitespace-pre-line text-zinc-300">${esc(job.requirements)}</div>
            </div>
          ` : ''}
      ...
      <div class="space-y-3">
        ${apps.map(app => `
          <div class="bg-[#14141e] border border-zinc-800 rounded-2xl p-4 flex items-center justify-between gap-4">
            <div class="space-y-1 min-w-0">
              <h4 class="text-sm font-bold text-white truncate">${esc(app.jobTitle || 'Job Application')}</h4>
              <p class="text-xs text-zinc-400">🏢 ${esc(app.companyName || 'Employer')}</p>
            </div>
...
async function renderEmployerDesk(container) {
  ...
  try {
    ...
    if (snap.empty) {
      ...
      `;
      $('deskPostJobBtn')?.addEventListener('click', () => openPostJobModal());
      return;
    }
          ...
          ${myJobs.map(job => `
            <div class="bg-[#14141e] border border-zinc-800 rounded-2xl p-4 space-y-3">
              <div class="flex items-start justify-between gap-3">
                <div>
                  <h4 class="text-base font-bold text-white">${esc(job.title)}</h4>
                  <p class="text-xs text-zinc-400">📍 ${esc(job.region)} · Category: ${esc(job.category)}</p>
                </div>
                <span class="px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase ${
    ...
    area.innerHTML = apps.map(app => `
      <div class="bg-[#12121a] border border-zinc-800 rounded-2xl p-4 space-y-2.5">
        <div class="flex items-center justify-between">
          <div>
            <h4 class="font-bold text-white text-sm">${esc(app.applicantName)}</h4>
            <p class="text-[11px] text-zinc-400">📧 ${esc(app.applicantEmail)} · 📞 ${esc(app.applicantPhone)}</p>
          </div>
          <span class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-300 border border-amber-500/30">
            ${esc(app.status || 'submitted')}
          </span>
        ...
        <div class="flex items-center justify-between pt-2 border-t border-zinc-800/60">
          ${app.cvStoragePath ? `
            <a href="${esc(app.cvStoragePath)}" target="_blank" class="text-amber-400 hover:underline font-bold text-xs flex items-center gap-1">
              <span>📄</span><span>Download CV (${esc(app.cvFileName || 'resume')})</span>
            </a>
    ...
    <div class="space-y-4">
      <div class="grid grid-cols-1 sm:grid-cols-12 gap-3">
        <div class="sm:col-span-8 relative">
          <input type="text" id="eventSearchInput" value="${esc(activeEventSearch)}" placeholder="Search event title, venue, organizer..." class="w-full bg-[#14141e] border border-zinc-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-zinc-500 focus:border-gold/60 focus:outline-none" />
        </div>
...
async function loadEventsFeed() {
  const list = $('eventsListContainer');
  if (!list) return;
      ...
      <div class="bg-[#14141e] hover:bg-[#181826] border border-zinc-800/80 hover:border-amber-500/40 rounded-2xl overflow-hidden transition shadow-sm flex flex-col justify-between">
        ...
        <div class="p-4 space-y-2.5 flex-1 flex flex-col justify-between">
          <div class="space-y-1">
            ...
            <p class="text-xs text-zinc-400">👤 By ${esc(ev.organizerName)}</p>
          </div>

          <div class="space-y-1 text-xs text-zinc-400 pt-2 border-t border-zinc-800/60">
            <div class="flex items-center gap-1.5">
              <span>📅</span> <span>${esc(ev.startDate || 'Upcoming')} at ${esc(ev.startTime || '09:00')}</span>
            </div>
            <div class="flex items-center gap-1.5 truncate">
              <span>📍</span> <span class="truncate">${esc(ev.venue || ev.city || 'Accra')}</span>
            </div>
          </div>
      ...
      <div class="flex-1 overflow-y-auto pr-1 space-y-4 text-xs text-zinc-300">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5 p-3 rounded-2xl bg-[#12121a] border border-zinc-800 text-xs">
          <div>
            ...
            <span class="font-bold text-white">${esc(ev.startDate || 'Upcoming')} at ${esc(ev.startTime || '09:00')}</span>
          </div>
          <div>
            <span class="text-zinc-500 block text-[10px]">Location</span>
            <span class="font-bold text-white">${esc(ev.region || 'Greater Accra')}</span>
          </div>
          <div>
        ...
        <div>
          ...
          <p class="text-zinc-400 bg-[#12121a] p-3 rounded-xl border border-zinc-800">${esc(ev.venue || 'Accra, Ghana')}</p>
        </div>

        <div>
          <h4 class="font-bold text-zinc-300 text-xs mb-1">📝 About This Event</h4>
          <p class="text-zinc-300 leading-relaxed whitespace-pre-wrap">${esc(ev.description || 'No additional description provided.')}</p>
        </div>
      </div>
      ...
      <form id="createEventForm" class="flex-1 overflow-y-auto pr-2 space-y-3.5 text-xs text-zinc-300">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <input type="text" id="eventOrganizer" required value="${esc(currentProfile?.name || '')}" class="w-full bg-[#12121a] border border-zinc-800 rounded-xl px-3 py-2 text-white text-xs focus:border-gold/60 focus:outline-none" />
          </div>
          <div>
            <label class="block font-bold text-zinc-300 mb-1">Category *</label>
  ...
  modal.querySelector('#createEventForm').onsubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        organizerName: $('eventOrganizer').value.trim(),
        category: $('eventCategory').value,
        startDate: $('eventDate').value,
        startTime: $('eventTime').value,
        region: $('eventRegion').value,
        venue: $('eventVenue').value.trim(),
        ticketType: $('eventTicketType').value,
        ticketPrice: Number($('eventPrice').value) || 0,
...
async function openRegisterEventModal(eventId) {
    ...
    <div class="bg-[#181824] border border-gold/40 rounded-3xl max-w-md w-full p-6 text-white shadow-2xl space-y-4">
      <div class="flex items-center justify-between pb-2 border-b border-zinc-800">
        <div>
          ...
          <p class="text-xs text-amber-300 font-semibold">${esc(eventData?.title || 'Event')}</p>
        </div>
      ...
      <form id="registerEventForm" class="space-y-3 text-xs text-zinc-300">
        <div>
          ...
          <input type="text" id="regName" required value="${esc(currentProfile?.name || currentUser?.displayName || '')}" class="w-full bg-[#12121a] border border-zinc-800 rounded-xl px-3 py-2 text-white text-xs focus:border-gold/60 focus:outline-none" />
        </div>

        <div>
          <label class="block font-bold text-zinc-300 mb-1">Email Address *</label>
          <input type="email" id="regEmail" required value="${esc(currentUser?.email || '')}" class="w-full bg-[#12121a] border border-zinc-800 rounded-xl px-3 py-2 text-white text-xs focus:border-gold/60 focus:outline-none" />
        </div>
  ...
  modal.querySelector('#registerEventForm').onsubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        eventId,
        attendeeName: $('regName').value.trim(),
        attendeeEmail: $('regEmail').value.trim(),
        attendeePhone: $('regPhone').value.trim(),
        ticketCount: 1
      };
    ...
    container.innerHTML = `
      <div class="space-y-3">
        ${regs.map(r => `
          <div class="bg-[#14141e] border border-zinc-800 rounded-2xl p-4 flex items-center justify-between gap-4">
            <div class="space-y-1">
              <h4 class="text-sm font-bold text-white">${esc(r.eventTitle || 'Event Ticket')}</h4>
              <p class="text-xs text-zinc-400">👤 Attendee: ${esc(r.attendeeName)} · 🎟️ ${r.ticketCount || 1} Ticket</p>
            </div>
...
async function renderOrganizerDesk(container) {
  ...
  try {
    ...
    if (snap.empty) {
      ...
      `;
      $('deskCreateEvBtn')?.addEventListener('click', () => openCreateEventModal());
      return;
    }
        ...
        <div class="space-y-3">
          ${myEvents.map(ev => `
            <div class="bg-[#14141e] border border-zinc-800 rounded-2xl p-4 space-y-2">
              <div class="flex items-start justify-between gap-3">
                <div>
                  <h4 class="text-base font-bold text-white">${esc(ev.title)}</h4>
                  <p class="text-xs text-zinc-400">📅 ${esc(ev.startDate)} · 📍 ${esc(ev.venue)}</p>
                </div>
                <span class="px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase ${
...
async function renderAdminJobsDesk(tabContainer) {
  ...
  $('refreshJobsDeskBtn')?.addEventListener('click', () => loadAdminQueues());
  loadAdminQueues();
}
